...
async function connect(options={}){
  ...
  connecting=(async()=>{const cfg=getConfig();if(!validConfig(cfg)){dispatch('otthos:mp-status',status({mode:'offline',error:'Firebase não configurado'}));return false}
    ...
    try{
      ...
      refs.slot=reservation.slotRef;refs.presence=f.ref(db,`${ROOT}/${WORLD}/presence/${user.uid}`);refs.presences=f.ref(db,`${ROOT}/${WORLD}/presence`);refs.profile=f.ref(db,`${ROOT}/users/${user.uid}/profile`);refs.progress=f.ref(db,`${ROOT}/users/${user.uid}/progress`);refs.learning=f.ref(db,`${ROOT}/users/${user.uid}/learning`);refs.guardian=f.ref(db,`${ROOT}/users/${user.uid}/guardianSettings`);refs.activityAudit=f.ref(db,`${ROOT}/users/${user.uid}/activityAudit`);refs.blocks=f.ref(db,`${ROOT}/users/${user.uid}/blocks`);refs.houses=f.ref(db,`${ROOT}/${WORLD}/houses`);refs.campfires=f.ref(db,`${ROOT}/${WORLD}/campfires`);refs.extensions=f.ref(db,`${ROOT}/${WORLD}/houseExtensions`);refs.boats=f.ref(db,`${ROOT}/${WORLD}/boats`);refs.chat=f.ref(db,`${ROOT}/${WORLD}/chat`);refs.gifts=f.ref(db,`${ROOT}/users/${user.uid}/inbox`);refs.interactions=f.ref(db,`${ROOT}/users/${user.uid}/interactions`);refs.challenges=f.ref(db,`${ROOT}/users/${user.uid}/challenges`);refs.socialRequests=f.ref(db,`${ROOT}/users/${user.uid}/socialRequests`);refs.sessions=f.ref(db,`${ROOT}/${WORLD}/gameSessions`);refs.coopMissions=f.ref(db,`${ROOT}/${WORLD}/coopMissions`);refs.gmGrants=f.ref(db,`${ROOT}/gmGrants/${user.uid}`);refs.gmReceipts=f.ref(db,`${ROOT}/gmGrantReceipts/${user.uid}`);lastPresence={name:neutralPublicName(user.uid),room:currentRoom(),x:0,y:0,z:0,r:0,color:0x5ad8ff};
...
async function gmListUsers(){
  try{const permission=await isCurrentUserGM(true);if(!permission.ok||!permission.allowed)return{ok:false,error:permission.error||'Conta sem permissão GM.'};const f=await ensureServices(),usersSnap=await f.get(f.ref(db,`${ROOT}/users`)),raw=usersSnap.val()||{},online={};await Promise.all(configuredRooms().map(async room=>{try{const snap=await f.get(f.ref(db,`${ROOT}/rooms/${room.id}/presence`));for(const[uid,value]of Object.entries(snap.val()||{})){if(presenceIsFresh(value))online[uid]={online:true,room:room.id,presence:value};}}catch{}}));const users=Object.entries(raw).map(([uid,value])=>{const userValue=value||{},progress=userValue.progress||{},profile={...(progress.profile||{}),...(userValue.profile||{})},presence=online[uid]||{};return{uid,name:sanitizePublicName(profile.name||neutralPublicName(uid)),username:String(userValue.profile?.username||'').slice(0,20),accountLinked:!!userValue.profile?.accountLinked,level:Number(profile.level||1),xp:Number(profile.xp||0),coins:Number(profile.coins||0),reputation:Number(profile.reputation||0),inventory:progress.inventory||{},lastSaved:Number(progress.lastSaved||0),online:!!presence.online,room:presence.room||'',profile,progress};}).sort((a,b)=>Number(b.online)-Number(a.online)||a.name.localeCompare(b.name,'pt-BR'));return{ok:true,users};}catch(error){return{ok:false,error:error?.message||String(error),users:[]}}
}
async function gmReadUser(targetUid){
  targetUid=String(targetUid||'');if(!validPublicUid(targetUid))return{ok:false,error:'UID de jogador inválido.'};try{const permission=await isCurrentUserGM();if(!permission.ok||!permission.allowed)return{ok:false,error:permission.error||'Conta sem permissão GM.'};const f=await ensureServices(),[userSnap,grantsSnap,receiptsSnap]=await Promise.all([f.get(f.ref(db,`${ROOT}/users/${targetUid}`)),f.get(f.ref(db,`${ROOT}/gmGrants/${targetUid}`)),f.get(f.ref(db,`${ROOT}/gmGrantReceipts/${targetUid}`))]);return{ok:true,uid:targetUid,user:userSnap.val()||{},grants:grantsSnap.val()||{},receipts:receiptsSnap.val()||{}};}catch(error){return{ok:false,error:error?.message||String(error)}}
}
