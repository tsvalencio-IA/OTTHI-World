...
ck('Colisão local não causa reset completo imediatamente','safePointNear(player.x,player.z' in recovery and 'Math.hypot(local.x-player.x,local.z-player.z)<=2.2' in recovery)
...
ck('Rodovia possui culling por distância','function updateOttoviasVisibility' in ott and 'item.group.visible=nearest.distance<range' in ott and 'updateOttoviasVisibility();' in ott)
...
ck('Release V705.5',ver.get('hotfix')=='705.5' and ver.get('assetVersion')==7055)
