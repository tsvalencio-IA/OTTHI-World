...
ck('Colisão local nunca cai em recuperação distante','safePointNear(player.x,player.z' in recovery and 'Math.hypot(local.x-player.x,local.z-player.z)<=1.45' in recovery and 'player.invalidSince=0;return false;' in recovery and "const reason=outside?'fora dos limites':belowWorld?'abaixo do terreno':deepFall?'queda sem retorno':''" in recovery)
