...
// @otthi-module-body
  ...
  function recommendedEducationTarget(){const entries=Object.keys(EDUCATION_SUBJECTS).map(id=>educationSubjectSnapshot(id));entries.sort((a,b)=>a.completed-b.completed||a.xp-b.xp||a.pct-b.pct);const chosen=entries[0]||educationSubjectSnapshot('math');return{subject:chosen.id,level:Math.max(1,chosen.nextLevel||1),meta:chosen};}
  function thamisWelcomeLine(){const rec=recommendedEducationTarget(),playerName=playerDisplayName();return`Oi, ${playerName}! Eu sou a ${OTTON_BRAND.mentor}, gerente e professora da ${OTTON_BRAND.school}. Seu próximo passo ideal agora é <b>${escapeHtml(rec.meta.title)}</b>, nível <b>${rec.level}</b>.`;}
  function thamisPlanItems(){const rec=recommendedEducationTarget(),summary=educationSummary();return[{icon:rec.meta.icon,title:`Foco principal: ${rec.meta.title}`,text:`Avance no nível ${rec.level} para manter a sequência progressiva e fortalecer a autonomia.`,subject:rec.subject,level:rec.level,kind:'play'},{icon:'🎯',title:'Plano do dia',text:`Conclua os desafios diários e mantenha a sequência em ${state.daily.streak||1} dia(s).`,kind:'daily'},{icon:'📘',title:`Revisão guiada • ${summary.pct}%`,text:`Abra a trilha completa do ${OTTON_BRAND.platform} e revise a matéria mais fraca quando quiser.`,kind:'hub'}];}
  function thamisMethodHtml(){const pillars=[['Progressão individual','Cada criança avança em passos curtos, começando do ponto em que consegue ter sucesso.'],['Repetição inteligente','As atividades retomam a habilidade até ela ficar natural, rápida e segura.'],['Correção imediata','O erro vira orientação na hora, para reforçar raciocínio e leitura sem travar a brincadeira.'],['Autonomia com rotina','A proposta é estudar um pouco por vez, todos os dias, dentro do mundo aberto.']];return`<div class="thamis-pillar-grid">${pillars.map(([title,text])=>`<article class="thamis-card"><b>${escapeHtml(title)}</b><p>${escapeHtml(text)}</p></article>`).join('')}</div>`;}
  function thamisProgressHtml(){return`<div class="thamis-progress-grid">${Object.keys(EDUCATION_SUBJECTS).map(id=>{const item=educationSubjectSnapshot(id);return`<article class="thamis-card thamis-progress-card" style="--thamis:${item.color}"><div><small>${item.icon} ${escapeHtml(item.title)}</small><b>${item.completed}/${item.total} fases</b></div><div class="thamis-progress-bar"><i style="width:${item.pct}%"></i></div><p>${item.pct}% concluído • ${item.stars} estrelas • ${item.xp} XP</p><button class="btn primary" data-thamis-play="${item.id}" data-thamis-level="${item.nextLevel}">Continuar</button></article>`;}).join('')}</div>`;}
  function openTiaThamisHub(tab='welcome'){
    ensureDailyChallenges();const summary=educationSummary(),valid=(['welcome','plan','progress','method'].includes(tab)?tab:'welcome'),plan=thamisPlanItems();
    const tabs=`<div class="thamis-tabs"><button class="${valid==='welcome'?'active':''}" data-thamis-tab="welcome">👩‍🏫 Atendimento</button><button class="${valid==='plan'?'active':''}" data-thamis-tab="plan">🗂️ Plano</button><button class="${valid==='progress'?'active':''}" data-thamis-tab="progress">📈 Progresso</button><button class="${valid==='method'?'active':''}" data-thamis-tab="method">🧠 Método</button></div>`;
    const content=valid==='welcome'?`<section class="thamis-pane"><div class="thamis-highlight"><b>${OTTON_BRAND.platform}</b><p>${thamisWelcomeLine()}</p><div class="button-grid"><button data-thamis-recommended class="primary">▶️ Treinar recomendação</button><button data-thamis-daily>🎯 Desafios do dia</button><button data-thamis-fullhub>📚 Abrir trilha completa</button><button data-thamis-speak>🔊 Ouvir orientação</button></div></div><div class="academy-start">${Object.keys(EDUCATION_SUBJECTS).map(id=>{const s=EDUCATION_SUBJECTS[id];return`<button data-thamis-play="${id}" data-thamis-level="1"><span>${s.icon}</span><b>${escapeHtml(s.title)}</b><small>${escapeHtml(s.description)}</small></button>`;}).join('')}</div></section>`:valid==='plan'?`<section class="thamis-pane"><div class="thamis-card-grid">${plan.map((item,index)=>`<article class="thamis-card"><small>${item.icon} ETAPA ${index+1}</small><b>${escapeHtml(item.title)}</b><p>${escapeHtml(item.text)}</p>${item.kind==='play'?`<button class="btn primary" data-thamis-play="${item.subject}" data-thamis-level="${item.level}">Começar agora</button>`:item.kind==='daily'?`<button class="btn primary" data-thamis-daily>Abrir desafios</button>`:`<button class="btn primary" data-thamis-fullhub>Ver trilha</button>`}</article>`).join('')}</div></section>`:valid==='progress'?`<section class="thamis-pane">${thamisProgressHtml()}</section>`:`<section class="thamis-pane"><div class="thamis-card"><small>${OTTON_BRAND.method}</small><b>Como a Tia Thamis organiza o aprendizado</b><p>O ${OTTON_BRAND.platform} usa desafios curtos de matemática, português e inglês para treinar fluência, atenção e autonomia dentro do mundo do Otto.</p></div>${thamisMethodHtml()}</section>`;
    openModal(`${OTTON_BRAND.mentor} • ${OTTON_BRAND.platform}`,`<div class="thamis-console"><div class="academy-banner thamis-banner"><div>👩‍🏫</div><section><b>${OTTON_BRAND.method}</b><span>Gerência da escola, rotina de estudos e trilha pedagógica do Otto.</span></section></div><div class="learning-top"><div><b>🔥 ${state.daily.streak||1}</b><small>sequência</small></div><div><b>👑 ${state.learning.crowns||0}</b><small>coroas</small></div><div><b>⭐ ${summary.pct}%</b><small>trilha</small></div><div><b>🏆 ${state.learning.perfectLessons||0}</b><small>perfeitas</small></div></div>${tabs}<div class="thamis-content">${content}</div></div>`,root=>{
      root.onclick=e=>{const tabBtn=e.target.closest('[data-thamis-tab]'),playBtn=e.target.closest('[data-thamis-play]'),dailyBtn=e.target.closest('[data-thamis-daily]'),hubBtn=e.target.closest('[data-thamis-fullhub]'),recBtn=e.target.closest('[data-thamis-recommended]'),speakBtn=e.target.closest('[data-thamis-speak]');if(tabBtn){openTiaThamisHub(tabBtn.dataset.thamisTab);return;}if(playBtn){startSoloEducationGame(playBtn.dataset.thamisPlay,Number(playBtn.dataset.thamisLevel)||1);return;}if(dailyBtn){openEducationHub('daily');return;}if(hubBtn){openEducationHub(String(state.learning.lastLesson||'math').split('-')[0]);return;}if(recBtn){const rec=recommendedEducationTarget();startSoloEducationGame(rec.subject,rec.level);return;}if(speakBtn){try{speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(`Olá, ${playerDisplayName()}. ${thamisWelcomeLine().replace(/<[^>]+>/g,'')}`);u.lang='pt-BR';u.rate=.94;u.pitch=1.04;speechSynthesis.speak(u);}catch{toast('Áudio não disponível neste aparelho.','warn',1800);}return;}};
    });
  }
  function openEducationHub(tab='math'){
    ...
    const startCards=`<div class="academy-start"><button data-academy-start="math"><span>🔢</span><b>Treinar Matemática</b><small>Contagem, soma e lógica</small></button><button data-academy-start="portuguese"><span>📚</span><b>Treinar Português</b><small>Letras, sílabas e leitura</small></button><button data-academy-start="english"><span>🌎</span><b>Treinar English</b><small>Imagens, palavras e áudio</small></button></div>`;
  ...
  window.OTTHI_TIA_THAMIS={open:openTiaThamisHub,recommend:recommendedEducationTarget,plan:thamisPlanItems};
