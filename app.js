(() => {
  ...
  // ===== MODULE: 00a-visual-foundation-avatar-v2.js =====
  ...
  // ===== MODULE: 13a-sports-kart-v705.js =====
  ...
  function createSportsComplexV704(){
    ...
    for(let i=0;i<112;i++){const a=i/112*Math.PI*2,b=(i+1)/112*Math.PI*2,m=(a+b)/2,ax=cx+Math.cos(a)*rx,az=cz+Math.sin(a)*rz,bx=cx+Math.cos(b)*rx,bz=cz+Math.sin(b)*rz,len=Math.hypot(bx-ax,bz-az),tanX=bx-ax,tanZ=bz-az,n=Math.max(.001,Math.hypot(tanX,tanZ)),nx=-tanZ/n,nz=tanX/n,road=new THREE.Mesh(new THREE.BoxGeometry(len+.14,.09,7.2),track);road.position.set((ax+bx)/2,.155,(az+bz)/2);road.rotation.y=Math.atan2(tanX,tanZ);road.receiveShadow=true;worldGroup.add(road);for(const offset of[-2.7,-.9,.9,2.7]){const stripe=new THREE.Mesh(new THREE.BoxGeometry(len+.12,.016,.045),white);stripe.position.set(road.position.x+nx*offset,.211,road.position.z+nz*offset);stripe.rotation.y=road.rotation.y;worldGroup.add(stripe);}}
    ...
    const hub=worldLayoutPoint('sportsEntrance',{x:cx,z:cz-22}),football=v705FootballEntrance(sports);createSignpost(hub.x,hub.z+.6,'🏟️ ESPORTES',Math.PI);createSignpost(football.x,football.z+.8,'⚽ FUTEBOL — JOGAR',Math.PI);premiumBox(2.6,.035,8.5,0xd9d2bb,(hub.x+football.x)/2,.11,(hub.z+football.z)/2,worldGroup);
  ...
  function createCourtV705(type,center,foot=false){const cx=center.x,cz=center.z,w=12,d=20,base=foot?0xd9b576:0x4597ce;premiumBox(w,.1,d,base,cx,.08,cz,worldGroup);v705FieldLine(w,.1,cx,cz-d/2);v705FieldLine(w,.1,cx,cz+d/2);v705FieldLine(.1,d,cx-w/2,cz);v705FieldLine(.1,d,cx+w/2,cz);v705FieldLine(w,.07,cx,cz);const netH=foot?1.85:2.3;for(let y=.35;y<netH;y+=.26)premiumBox(w,.022,.022,0xf4f7fb,cx,y,cz,worldGroup);premiumBox(.14,netH+.3,.14,0xe7ecf2,cx-w/2,.5+netH/2,cz,worldGroup);premiumBox(.14,netH+.3,.14,0xe7ecf2,cx+w/2,.5+netH/2,cz,worldGroup);const label=foot?'Futevôlei':'Vôlei',entry=worldLayoutPoint(foot?'footvolleyEntrance':'volleyEntrance',{x:cx,z:cz-d/2-1}),c={id:`${type}-v705`,type,label,cx,cz,w,d,netH,foot,ball:null,shadow:null,team:[],opponents:[],score:[0,0],serving:0,state:'idle',touches:[0,0],lastSide:-1,timer:0,vx:0,vy:0,vz:0,pointLock:0,lastHintAt:0};SPORTS_V705.courts.set(type,c);createSignpost(entry.x,entry.z+.5,`${foot?'🏖️':'🏐'} ${label.toUpperCase()} — JOGAR`,Math.PI);registerInteractable({id:`${type}-v705`,type:'sport',icon:foot?'🏖️':'🏐',label:`${label} — jogar partida`,x:entry.x,z:entry.z,radius:4.8,priority:185,action:()=>startCourtV704(type)});}
  function openFootballV705(){openModal('⚽ Futebol OTTHI',`<p><b>Partida 4 × 4.</b> Você joga com dois companheiros e goleiro contra três adversários e goleiro.</p><p><b>Joystick</b> conduz · <b>AÇÃO</b> chuta/desarma · <b>PODER</b> passa para o companheiro melhor posicionado.</p><div class="button-grid"><button id="footballSoloV705" class="primary">⚽ Jogar contra o computador</button><button id="footballOnlineV705">🌐 Abrir partida da sala</button></div>`);setTimeout(()=>{const a=document.getElementById('footballSoloV705'),b=document.getElementById('footballOnlineV705');if(a)a.onclick=()=>startFootballV704('solo');if(b)b.onclick=()=>startFootballV704('online');},0);}
  ...
  function startFootballV704(mode='solo'){closeModal();stopSportV704(false);const s=world.sportsV704,cx=s.centerX,cz=s.centerZ,ball=v705Ball(0xffffff,.46);ball.position.set(cx,.52,cz);const home=[v705Athlete(0x2f7ed6,cx-7,cz-4,'Ala azul','field',1.02),v705Athlete(0x2f7ed6,cx-3,cz+4,'Meia azul','field',.98),v705Athlete(0x2f7ed6,cx-s.fieldW/2+1.7,cz,'Goleiro azul','keeper',1)],away=[v705Athlete(0xe5484d,cx+5,cz-5,'Marcador vermelho','field',1.02),v705Athlete(0xe5484d,cx+7,cz+4,'Ala vermelho','field',.98),v705Athlete(0xcf3340,cx+11,cz,'Atacante vermelho','field',1.05),v705Athlete(0xf0a22e,cx+s.fieldW/2-1.7,cz,'Goleiro vermelho','keeper',1)];SPORTS_V705.football={type:'football',mode,cx,cz,w:s.fieldW,d:s.fieldD,ball,home,away,score:[0,0],time:150,vx:0,vz:0,cooldown:1,finished:false,possession:-1,countdown:3.2,lastOutAt:0,lastTackleAt:0};SPORTS_V705.active=SPORTS_V705.football;player.x=cx-9;player.z=cz+1.5;player.y=v705GroundY(player.x,player.z);player.vx=player.vz=0;v705SetPrimaryAction('⚽','Chutar');v705SetSpecial('↗️','Passe');v705Hud('⚽ Futebol OTTHI','3');toast('Valendo em 3 segundos. Conduza, passe e procure espaço para chutar.','good',3000);}
  function v705FootballKick(power=10.2,target=null){const f=SPORTS_V705.football;if(!f||f.finished||f.countdown>0)return false;const d=Math.hypot(f.ball.position.x-player.x,f.ball.position.z-player.z);if(d>2.3)return false;let fx=Math.sin(player.facing),fz=Math.cos(player.facing);if(target){const dx=target.group.position.x-f.ball.position.x,dz=target.group.position.z-f.ball.position.z,n=Math.max(.001,Math.hypot(dx,dz));fx=dx/n;fz=dz/n;}else{const gx=f.cx+f.w/2+1,gz=f.cz,dx=gx-f.ball.position.x,dz=gz-f.ball.position.z,n=Math.max(.001,Math.hypot(dx,dz));fx=fx*.68+dx/n*.32;fz=fz*.68+dz/n*.32;const m=Math.max(.001,Math.hypot(fx,fz));fx/=m;fz/=m;}f.vx=fx*power+(player.vx||0)*.2;f.vz=fz*power+(player.vz||0)*.2;f.cooldown=.25;f.possession=-1;return true;}
  function footballKickV704(){const f=SPORTS_V705.football;if(!f)return false;const ball=f.ball.position,d=Math.hypot(ball.x-player.x,ball.z-player.z);if(d>2.3){const enemy=f.away.filter(a=>a.role==='field').sort((a,b)=>Math.hypot(player.x-a.group.position.x,player.z-a.group.position.z)-Math.hypot(player.x-b.group.position.x,player.z-b.group.position.z))[0];if(enemy&&Math.hypot(player.x-enemy.group.position.x,player.z-enemy.group.position.z)<1.45&&Math.hypot(ball.x-enemy.group.position.x,ball.z-enemy.group.position.z)<1.7){const dx=ball.x-player.x,dz=ball.z-player.z,n=Math.max(.001,Math.hypot(dx,dz));f.vx=dx/n*5.6;f.vz=dz/n*5.6;f.cooldown=.35;toast('Desarme!','good',650);return true;}return false;}const ok=v705FootballKick(11.4);if(ok)toast('Chute!','good',600);return ok;}
  function v705FootballPass(){const f=SPORTS_V705.football;if(!f)return false;const candidates=f.home.filter(a=>a.role==='field'),mate=candidates.sort((a,b)=>{const av=(a.group.position.x-f.ball.position.x)*1.5-Math.abs(a.group.position.z-f.ball.position.z),bv=(b.group.position.x-f.ball.position.x)*1.5-Math.abs(b.group.position.z-f.ball.position.z);return bv-av;})[0];const ok=v705FootballKick(7.8,mate);if(ok)toast(`Passe para ${mate.label}!`,'good',650);return ok;}
  ...
  function updateFootballV704(dt){const f=SPORTS_V705.football;if(!f||f.finished)return;if(f.countdown>0){f.countdown=Math.max(0,f.countdown-dt);f.ball.position.set(f.cx,.52,f.cz);f.vx=f.vz=0;v705Hud('⚽ Futebol OTTHI',f.countdown>0?String(Math.ceil(f.countdown)):'VALENDO!');if(f.countdown===0)toast('Valendo!','good',800);return;}f.time=Math.max(0,f.time-dt);f.cooldown=Math.max(0,f.cooldown-dt);for(const a of f.home.concat(f.away))a.cooldown=Math.max(0,a.cooldown-dt);const ball=f.ball.position;ball.x+=f.vx*dt;ball.z+=f.vz*dt;const fr=Math.pow(.15,dt);f.vx*=fr;f.vz*=fr;f.ball.rotation.z-=f.vx*dt;f.ball.rotation.x+=f.vz*dt;const pd=Math.hypot(ball.x-player.x,ball.z-player.z),playerSpeed=Math.hypot(player.vx||0,player.vz||0);
    if(pd<1.28&&Math.hypot(f.vx,f.vz)<4.4&&f.cooldown<=0){const tx=player.x+Math.sin(player.facing)*.9,tz=player.z+Math.cos(player.facing)*.9;ball.x=lerp(ball.x,tx,Math.min(1,dt*11));ball.z=lerp(ball.z,tz,Math.min(1,dt*11));f.vx=(player.vx||0)*.7;f.vz=(player.vz||0)*.7;f.possession=0;}else f.possession=-1;
  ...
  function startCourtV704(type){stopSportV704(false);closeModal();const c=SPORTS_V705.courts.get(type);if(!c)return;c.ball=c.ball||v705Ball(c.foot?0xffd24b:0xffffff,.39);if(!c.shadow){c.shadow=new THREE.Mesh(new THREE.CircleGeometry(.5,18),renderMat(0x111827,{transparent:true,opacity:.25,roughness:1,side:THREE.DoubleSide}));c.shadow.geometry.rotateX(-Math.PI/2);worldGroup.add(c.shadow);}if(!c.team.length)c.team=[v705Athlete(0x2f7ed6,c.cx-2,c.cz-4.5,`${c.label} parceiro`,'field',1.02)];if(!c.opponents.length)c.opponents=[v705Athlete(0x8b5cf6,c.cx-2,c.cz+4.5,`${c.label} CPU 1`,'field',1.0),v705Athlete(0xd66083,c.cx+2,c.cz+6,`${c.label} CPU 2`,'field',.97)];c.score=[0,0];c.touches=[0,0];c.lastSide=-1;c.serving=0;c.state='serve';c.timer=180;c.vx=c.vy=c.vz=0;c.pointLock=0;c.ball.position.set(c.cx,.75,c.cz-6);player.x=c.cx+2;player.z=c.cz-6;player.y=v705GroundY(player.x,player.z);player.vx=player.vz=0;SPORTS_V705.active=c;v705SetPrimaryAction(c.foot?'🦶':'🏐',c.foot?'Dominar/Atacar':'Sacar/Atacar');v705SetSpecial('⬆️','Levantar');v705Hud(`${c.foot?'🏖️':'🏐'} ${c.label}`,'AÇÃO para sacar');toast(`${c.label}: mova-se até a sombra da bola. AÇÃO recebe/ataca e PODER levanta.`, 'good',3200);}
  ...
  function updateCourtV704(c,dt){if(!c||SPORTS_V705.active!==c)return;c.timer=Math.max(0,c.timer-dt);c.pointLock=Math.max(0,c.pointLock-dt);if(c.state==='serve'){v705Hud(`${c.foot?'🏖️':'🏐'} ${c.label}`,`${c.serving===0?'AÇÃO para sacar':'Saque do computador'} • ${c.score[0]} × ${c.score[1]}`);return;}c.vy-=9.2*dt;c.ball.position.x+=c.vx*dt;c.ball.position.y+=c.vy*dt;c.ball.position.z+=c.vz*dt;if(c.shadow){c.shadow.visible=true;c.shadow.position.set(c.ball.position.x,.24,c.ball.position.z);const scale=clamp(1.35-c.ball.position.y*.08,.55,1.18);c.shadow.scale.setScalar(scale);c.shadow.material.opacity=clamp(.4-c.ball.position.y*.045,.1,.34);}c.ball.rotation.x+=c.vz*dt;c.ball.rotation.z-=c.vx*dt;if(Math.abs(c.ball.position.z-c.cz)<.26&&c.ball.position.y<c.netH+.13){c.ball.position.z=c.cz+(c.vz>0?-.3:.3);c.vz*=-.42;c.vy*=.58;}
  ...
  function createKartCircuitV704(){if(SPORTS_V705.kart)return SPORTS_V705.kart;const c=worldLayoutPoint('kart'),cx=c.x,cz=c.z,local=[[17,-7],[12,-13.8],[4,-15.4],[-4,-12.8],[-11,-14.6],[-18,-10],[-18,-3],[-12,2],[-17,8],[-10,13.7],[-2,13.8],[3,10],[7,14.5],[14,11.8],[18,5],[13,0],[18,-3],[12,-6]],points=local.map(([x,z])=>new THREE.Vector3(cx+x,.16,cz+z)),curve=new THREE.CatmullRomCurve3(points,true,'centripetal',.42),trackMat=renderMat(0x30343b,{roughness:.9}),curbA=renderMat(0xffffff,{roughness:.75}),curbB=renderMat(0xe34b45,{roughness:.75}),root=new THREE.Group();root.name='OTTHI_V705_KART';worldGroup.add(root);premiumBox(44,.08,38,0x477a3c,cx,.04,cz,worldGroup);const segs=190,width=5.0;for(let i=0;i<segs;i++){const t=i/segs,t2=(i+1)/segs,p=curve.getPointAt(t),q=curve.getPointAt(t2),tan=curve.getTangentAt((t+t2)/2),len=p.distanceTo(q),road=new THREE.Mesh(new THREE.BoxGeometry(len+.18,.1,width),trackMat);road.position.copy(p.clone().add(q).multiplyScalar(.5));road.position.y=.15;road.rotation.y=Math.atan2(tan.x,tan.z);road.userData.kartTrack=true;worldGroup.add(road);const nx=-tan.z,nz=tan.x;for(const side of[-1,1]){const curb=new THREE.Mesh(new THREE.BoxGeometry(len+.12,.14,.32),i%2?curbA:curbB);curb.position.set(road.position.x+nx*side*(width/2+.05),.21,road.position.z+nz*side*(width/2+.05));curb.rotation.y=road.rotation.y;worldGroup.add(curb);if(i%4===0){const holder=new THREE.Group();holder.position.set(road.position.x+nx*side*(width/2+.68),0,road.position.z+nz*side*(width/2+.68));holder.rotation.y=road.rotation.y;holder.userData.kartBarrier=true;worldGroup.add(holder);box(len+.08,.48,.18,0xe6e8ec,0,.3,0,holder);}}}
    ...
    const checkpoints=Array.from({length:14},(_,i)=>{const t=i/14,p=v705CurvePoint(curve,t);return{x:p.x,z:p.z,t};}),boosts=[.18,.52,.80].map(t=>v705CreateBoostPad({curve},t)),grid=[];for(let i=0;i<4;i++){const back=1.6+i*1.9,side=(i%2?1:-1)*.95;grid.push({x:start.x-tan.x*back+nx*side,z:start.z-tan.z*back+nz*side,heading});}const colors=[0xe5484d,0x2787d8,0xf1c943,0x31a76a],karts=grid.map((g,i)=>v705KartVehicle(`kart-${['red','blue','yellow','green'][i]}`,g.x,g.z,colors[i],g.heading)),k={cx,cz,curve,checkpoints,boosts,grid,karts,active:false,countdown:0,lap:0,expected:1,startAt:0,lapStartAt:0,best:0,ai:[],root,offTrackWarnAt:0,ranking:[],boostMessageAt:0};SPORTS_V705.kart=k;world.kartV704=k;const entrance=worldLayoutPoint('kartEntrance');registerInteractable({id:'kart-race-v705',type:'kart',icon:'🏁',label:'Kartódromo — corrida oficial',x:entrance.x,z:entrance.z,radius:4.0,priority:190,action:()=>startKartRaceV704()});createSignpost(entrance.x,entrance.z+1.2,'🏁 KARTÓDROMO — CORRIDA',Math.PI);return k;}
  function startKartRaceV704(){const k=SPORTS_V705.kart;if(!k||k.active)return;let kart=currentVehicleRef?.();if(!kart||kart.kind!=='kart'){kart=k.karts.find(v=>!v.occupied)||k.karts[0];enterVehicle(kart);}if(!player.vehicle||currentVehicleRef?.()?.kind!=='kart'){toast('Entre em um kart para correr.','warn');return;}const v=currentVehicleRef(),g=k.grid[0];v.group.position.set(g.x,0,g.z);v.group.rotation.y=g.heading;player.x=g.x;player.z=g.z;player.car.heading=g.heading;player.car.speed=0;k.active=true;k.countdown=3.4;k.lap=0;k.expected=1;k.startAt=performance.now();k.lapStartAt=k.startAt;k.best=0;k.ai=[];for(let i=0;i<3;i++){const gr=new THREE.Group(),gg=k.grid[i+1];gr.position.set(gg.x,0,gg.z);worldGroup.add(gr);box(1.38,.2,1.95,[0x7d58c9,0xf28a22,0x138d83][i],0,.3,0,gr);for(const [x,z]of[[-.72,-.62],[.72,-.62],[-.72,.62],[.72,.62]]){const w=cylinder(.27,.22,0x111827,x,.25,z,gr,12);w.rotation.z=Math.PI/2;}k.ai.push({group:gr,t:(.985-i*.012+1)%1,baseSpeed:.078+i*.0025,lap:0,finished:false,lane:(i-1)*.38,skill:.92+i*.035});}SPORTS_V705.active=k;v705Hud('🏁 Kart OTTHI','3');toast('3 voltas. Hairpins, chicanes e faixas azuis de turbo: acelere e escolha a linha certa.','good',3200);}
  function updateKartRaceV704(dt){const k=SPORTS_V705.kart;if(!k?.active)return;k.countdown-=dt;if(k.countdown>0){player.car.speed=0;v705Hud('🏁 Kart OTTHI',String(Math.ceil(k.countdown)));return;}const now=performance.now();for(const ai of k.ai){if(ai.finished)continue;const curvePenalty=clamp(v705KartCurvature(k,ai.t)*2.6,0,.42),nearPlayer=Math.hypot(player.x-ai.group.position.x,player.z-ai.group.position.z)<3.2?1.035:1;ai.t+=ai.baseSpeed*ai.skill*(1-curvePenalty)*nearPlayer*dt;if(ai.t>=1){ai.t-=1;ai.lap++;if(ai.lap>=3)ai.finished=true;}const p=k.curve.getPointAt(ai.t),tan=k.curve.getTangentAt(ai.t),nx=-tan.z,nz=tan.x;ai.group.position.set(p.x+nx*ai.lane,0,p.z+nz*ai.lane);ai.group.rotation.y=Math.atan2(tan.x,tan.z);}
    const cp=k.checkpoints[k.expected],d=cp?Math.hypot(player.x-cp.x,player.z-cp.z):Infinity;if(d<3.0){if(k.expected===0){k.lap++;const lapTime=(now-k.lapStartAt)/1000;k.lapStartAt=now;if(k.lap>1||lapTime>3)k.best=!k.best?lapTime:Math.min(k.best,lapTime);k.expected=1;toast(`Volta ${k.lap}/3 • ${lapTime.toFixed(1)}s`,'good',1100);if(k.lap>=3){k.active=false;const elapsed=(now-k.startAt)/1000,playerProgress=k.lap,aiBest=Math.max(...k.ai.map(a=>a.lap+(a.t||0))),won=playerProgress>=aiBest;addXP(won?65:30);if(won)addCoins(42);v705Hud('🏁 Kart OTTHI',`${elapsed.toFixed(1)}s • ${won?'1º lugar':'Chegada'} • melhor ${k.best.toFixed(1)}s`);toast(won?'Você venceu a corrida de kart!':'Corrida concluída.','good',2400);setTimeout(()=>{SPORTS_V705.active=null;v705HudOff();},1700);return;}}else{k.expected++;if(k.expected>=k.checkpoints.length)k.expected=0;}}
    ...
    const exactProgress=k.lap+track.t,kRanks=k.ai.map((a,i)=>({name:`CPU ${i+1}`,progress:a.lap+a.t}));kRanks.push({name:'Você',progress:exactProgress});kRanks.sort((a,b)=>b.progress-a.progress);const pos=kRanks.findIndex(r=>r.name==='Você')+1,boosting=now<Number(player.car.kartBoostUntil||0);v705Hud('🏁 Kart OTTHI',`Volta ${Math.min(3,k.lap+1)}/3 • ${pos}º/4 • ${boosting?'TURBO • ':''}CP ${k.expected}/${k.checkpoints.length-1}`);v705PublishSport({type:'kart',lap:k.lap,checkpoint:k.expected,position:pos,progress:+track.t.toFixed(3)});}
  ...
  function campfireAllowed(x,z){const camp=worldLayoutPoint('camp',{x:-78,z:-62});if(Math.hypot(x-camp.x,z-camp.z)>14)return false;if(waterAt(x,z))return false;if(WORLD_MAP_ROADS.some(r=>rectOverlap({x,z,w:3,d:3},r,.5)))return false;if(world.houses.some(h=>Math.hypot(x-h.x,z-h.z)<10))return false;return true;}
  ...
  function createCampfireZone(){const camp=worldLayoutPoint('camp',{x:-78,z:-62});for(let i=0;i<9;i++){const a=i/9*Math.PI*2;premiumBox(.75,.3,.5,0x8b765e,camp.x+Math.cos(a)*2,.15,camp.z+Math.sin(a)*2);}createSignpost(camp.x+5,camp.z+5,'Acampamento',Math.PI*.25);registerInteractable({id:'campfire-zone',type:'campfire-zone',icon:'🔥',label:'Área de fogueira',x:camp.x,z:camp.z,radius:5,priority:145,action:openCampfireZone});}
  ...
  function extensionValid(draft){const rect={x:Number(draft.x),z:Number(draft.z),w:5.4,d:5.4};if(Math.abs(rect.x)>111||Math.abs(rect.z)>111)return false;if(world.hazards.some(h=>rectOverlap(rect,h,.5)))return false;if(WORLD_MAP_ROADS.some(r=>rectOverlap(rect,r,.4)))return false;if(typeof v704ProtectedRectangles==='function'&&v704ProtectedRectangles().some(item=>item.id!==draft.houseId&&item.kind!=='construction'&&rectOverlap(rect,item,.55)))return false;if(world.houseExtensions.some(e=>rectOverlap(rect,{x:e.data.x,z:e.data.z,w:5.4,d:5.4},.35)))return false;for(const h of world.houses){if(h.id===draft.houseId)continue;if(rectOverlap(rect,{x:h.x,z:h.z,w:9,d:7},.6))return false;}return true;}
  ...
  function disposeFurnitureGroup(group){if(!group)return;group.parent?.remove(group);disposeDetachedVisual(group);}
  ...
  function buildWorld(){
    ...
    createToyCar(P('garage').x,P('garage').z,{id:'garage-orange',label:'Carro da Garagem',primary:0xf28a22,secondary:0x0aa7b8});
  ...
  function ensureOtthiWorldState(){
    ...
    state.avatar=normalizeAvatarV2({...state.avatar,renderMode:'otthi-world-v3',bodyStyle:state.avatar?.bodyStyle||'toy',face:state.avatar?.face||'face-happy-01',hair:state.avatar?.hair||'hair-short-01',torso:state.avatar?.torso||'world-jacket-01',legs:state.avatar?.legs||'world-pants-01',shoes:state.avatar?.shoes||'world-sneakers-01'});
    state.vehicles={...(state.vehicles||{}),modularParts:{...(state.vehicles?.modularParts||{})},partDurability:{...(state.vehicles?.partDurability||{})}};
  ...
  // ===== MODULE: 35-world-render-pbr-environment.js =====
  ...
  // ===== MODULE: 36-modular-build-machines.js =====
  ...
  // ===== MODULE: 36a-vehicle-damage-repair-v704.js =====
  ...
  function repairVehicleV704(idOrVehicle,vehicleMaybe=null){
    ...
    const missing=100-current,coins=Math.max(8,Math.ceil(missing*.42)),stone=Math.max(1,Math.ceil(missing/35)),wood=Math.max(1,Math.ceil(missing/50));if(state.profile.coins<coins||(state.inventory.stone||0)<stone||(state.inventory.wood||0)<wood){toast(`Reparo exige ${coins} moedas, ${stone} pedra(s) e ${wood} madeira(s).`,'warn',3000);return false;}
    addCoins(-coins);state.inventory.stone-=stone;state.inventory.wood-=wood;setVehicleDurabilityV704(vehicle||{id},100,'repair');state.vehicles.broken[id]=false;if(player.car.id===id)player.car.broken=false;saveState(true);updateHUD();toast('Veículo reparado e liberado para dirigir.','good',2200);return true;
  }
  function towVehicleToWorkshopV704(vehicle=nearestRepairableVehicleV704()){
    if(!vehicle){toast('Nenhum veículo encontrado para rebocar.','warn');return false;}const cost=20;if(state.profile.coins<cost){toast(`O reboque custa ${cost} moedas.`,'warn');return false;}if(player.vehicle&&currentVehicleRef?.()===vehicle)exitVehicle(true);addCoins(-cost);const w=worldLayoutPoint('workshop');vehicle.group.position.set(w.x+7,groundHeightAt(w.x+7,w.z),w.z-1);vehicle.group.rotation.y=Math.PI/2;vehicle.x=w.x+7;vehicle.z=w.z-1;vehicle.heading=Math.PI/2;persistParkedVehicle(vehicle);saveState(true);toast('Veículo rebocado para a oficina. Use a bancada para reparar.','good',2400);return true;
  }
  ...
  function createVehicleRepairStationV704(){if(world?.interactables?.some(item=>item.id==='vehicle-repair-v704'))return;const w=worldLayoutPoint('workshop');registerInteractable({id:'vehicle-repair-v704',type:'repair',icon:'🛠️',label:'Oficina — reparar ou montar veículo',x:w.x,z:w.z+4.8,radius:2.7,priority:235,action:()=>{const vehicle=nearestRepairableVehicleV704();if(vehicle&&vehicleDurabilityV704(vehicle)<100){openBrokenVehicleOptionsV704(vehicle);}else if(typeof openWorldModularGarage==='function')openWorldModularGarage(vehicle?.id||'');}});}
  ...
  // ===== MODULE: 38-otthi-world-integration-bootstrap.js =====
  ...
  function gmRenderUserList(){
    ...
    list.innerHTML=filtered.length?filtered.map(userRecord=>`<button class="gm-user-row ${userRecord.uid===gmSelectedUid?'active':''}" type="button" data-gm-user="${gmEscape(userRecord.uid)}"><span class="gm-online-dot ${userRecord.online?'online':''}" aria-label="${userRecord.online?'Online':'Offline'}"></span><span><b>${gmEscape(gmUserName(userRecord))}</b><small>${userRecord.username?`@${gmEscape(userRecord.username)}`:'Conta sem usuário'} • nível ${Number(userRecord.level||1)}</small><em>${gmEscape(userRecord.uid.slice(0,10))}…</em></span><strong>🪙 ${Number(userRecord.coins||0)}</strong></button>`).join(''):'<p class="gm-empty">Nenhum jogador encontrado.</p>';
