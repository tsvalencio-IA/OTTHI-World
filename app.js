(() => {
  ...
  function v705SportsHubV7051(){
    openModal('🏟️ Central de Esportes',`<p><b>Escolha o que você quer jogar.</b> Os locais também aparecem separados no GPS.</p><div class="sports-hub-grid"><button class="primary" data-v7051-sport="football">⚽ <b>Futebol</b><span>3 × 3 • chute • passe • goleiros</span></button><button data-v7051-sport="volley">🏐 <b>Vôlei</b><span>2 × 2 • saque • 3 toques</span></button><button data-v7051-sport="footvolley">🏖️ <b>Futevôlei</b><span>2 × 2 • chute/cabeceio</span></button><button data-v7051-sport="athletics">🏃 <b>Atletismo</b><span>corridas e voltas</span></button><button data-v7051-sport="kart">🏁 <b>Kartódromo</b><span>marcar pista no GPS</span></button></div>`,root=>{$$('[data-v7051-sport]',root).forEach(btn=>btn.onclick=()=>{const type=btn.dataset.v7051Sport;if(type==='football')return openFootballV705();if(type==='volley'||type==='footvolley')return startCourtV704(type);if(type==='athletics'){closeModal();return openRaceCenter();}if(type==='kart')return setWaypoint('kart');});});
  }
  ...
  function createSportsComplexV704(){
    ...
    registerInteractable({id:'sports-hub-v7051',type:'sport',icon:'🏟️',label:'Central de Esportes — escolher jogo',x:hub.x,z:hub.z,radius:5.2,priority:180,action:v705SportsHubV7051});
    registerInteractable({id:'athletics-gym',type:'race',icon:'🏃',label:'Atletismo — corridas e voltas',x:cx-8,z:cz-19,radius:4.2,priority:160,action:()=>openRaceCenter()});
    registerInteractable({id:'football-v705',type:'sport',icon:'⚽',label:'Futebol — jogar partida',x:football.x,z:football.z,radius:5.2,priority:190,action:()=>openFootballV705()});
    registerInteractable({id:'football-field-v7051',type:'sport',icon:'⚽',label:'Futebol — iniciar partida',x:cx,z:cz-sports.fieldD/2-1.2,radius:4.6,priority:188,action:()=>openFootballV705()});
    createCourtV705('volley',worldLayoutPoint('volley'),false);createCourtV705('footvolley',worldLayoutPoint('footvolley'),true);SPORTS_V705.initialized=true;return sports;
  }
  function createCourtV705(type,center,foot=false){const cx=center.x,cz=center.z,w=12,d=20,base=foot?0xd9b576:0x4597ce;premiumBox(w,.1,d,base,cx,.08,cz,worldGroup);v705FieldLine(w,.1,cx,cz-d/2);v705FieldLine(w,.1,cx,cz+d/2);v705FieldLine(.1,d,cx-w/2,cz);v705FieldLine(.1,d,cx+w/2,cz);v705FieldLine(w,.07,cx,cz);const netH=foot?1.85:2.3;for(let y=.35;y<netH;y+=.26)premiumBox(w,.022,.022,0xf4f7fb,cx,y,cz,worldGroup);premiumBox(.14,netH+.3,.14,0xe7ecf2,cx-w/2,.5+netH/2,cz,worldGroup);premiumBox(.14,netH+.3,.14,0xe7ecf2,cx+w/2,.5+netH/2,cz,worldGroup);const label=foot?'Futevôlei':'Vôlei',entry=worldLayoutPoint(foot?'footvolleyEntrance':'volleyEntrance',{x:cx,z:cz-d/2-1}),c={id:`${type}-v705`,type,label,cx,cz,w,d,netH,foot,ball:null,shadow:null,team:[],opponents:[],score:[0,0],serving:0,state:'idle',touches:[0,0],lastSide:-1,timer:0,vx:0,vy:0,vz:0,pointLock:0};SPORTS_V705.courts.set(type,c);createSignpost(entry.x,entry.z+.5,`${foot?'🏖️':'🏐'} ${label.toUpperCase()} — JOGAR`,Math.PI);registerInteractable({id:`${type}-v705`,type:'sport',icon:foot?'🏖️':'🏐',label:`${label} — jogar partida`,x:entry.x,z:entry.z,radius:4.6,priority:185,action:()=>startCourtV704(type)});}
  ...
  function openFootballV705(){openModal('⚽ Futebol OTTHI',`<p><b>Você encontrou o campo.</b> A partida é 3 × 3 e começa no gramado.</p><p><b>Joystick</b> corre e conduz a bola · <b>AÇÃO</b> chuta · <b>PODER</b> passa. Há companheiro, adversários e goleiros.</p><div class="button-grid"><button id="footballSoloV705" class="primary">⚽ Jogar agora contra o computador</button><button id="footballOnlineV705">🌐 Jogar com a sala</button></div>`);setTimeout(()=>{const a=document.getElementById('footballSoloV705'),b=document.getElementById('footballOnlineV705');if(a)a.onclick=()=>startFootballV704('solo');if(b)b.onclick=()=>startFootballV704('online');},0);}
  ...
  function startFootballV704(mode='solo'){closeModal();stopSportV704(false);const s=world.sportsV704,cx=s.centerX,cz=s.centerZ,ball=v705Ball(0xffffff,.46);ball.position.set(cx,.52,cz);const home=[v705Athlete(0x2f7ed6,cx-8,cz-4,'Companheiro','field'),v705Athlete(0x2f7ed6,cx-s.fieldW/2+1.7,cz,'Goleiro azul','keeper')],away=[v705Athlete(0xe5484d,cx+7,cz-4,'Adversário 1','field'),v705Athlete(0xe5484d,cx+9,cz+4,'Adversário 2','field'),v705Athlete(0xf0a22e,cx+s.fieldW/2-1.7,cz,'Goleiro vermelho','keeper')];SPORTS_V705.football={type:'football',mode,cx,cz,w:s.fieldW,d:s.fieldD,ball,home,away,score:[0,0],time:120,vx:0,vz:0,cooldown:1,finished:false,possession:-1,countdown:3.2,lastOutAt:0};SPORTS_V705.active=SPORTS_V705.football;player.x=cx-7;player.z=cz+3;player.y=v705GroundY(player.x,player.z);player.vx=player.vz=0;v705SetPrimaryAction('⚽','Chutar');v705SetSpecial('↗️','Passe');v705Hud('⚽ Futebol OTTHI','3');toast('Entre na jogada: conduza com o joystick, AÇÃO chuta e PODER passa.','good',3000);}
  function v705FootballKick(power=10.2,target=null){const f=SPORTS_V705.football;if(!f||f.finished||f.countdown>0)return false;const d=Math.hypot(f.ball.position.x-player.x,f.ball.position.z-player.z);if(d>2.45)return false;let fx=Math.sin(player.facing),fz=Math.cos(player.facing);if(target){const dx=target.group.position.x-f.ball.position.x,dz=target.group.position.z-f.ball.position.z,n=Math.max(.001,Math.hypot(dx,dz));fx=dx/n;fz=dz/n;}else{const gx=f.cx+f.w/2+1,gz=f.cz,dx=gx-f.ball.position.x,dz=gz-f.ball.position.z,n=Math.max(.001,Math.hypot(dx,dz));fx=fx*.7+dx/n*.3;fz=fz*.7+dz/n*.3;const m=Math.max(.001,Math.hypot(fx,fz));fx/=m;fz/=m;}f.vx=fx*power+player.vx*.2;f.vz=fz*power+player.vz*.2;f.cooldown=.28;f.possession=-1;return true;}
  ...
  function updateFootballV704(dt){const f=SPORTS_V705.football;if(!f||f.finished)return;if(f.countdown>0){f.countdown=Math.max(0,f.countdown-dt);f.ball.position.set(f.cx,.52,f.cz);f.vx=f.vz=0;v705Hud('⚽ Futebol OTTHI',f.countdown>0?String(Math.ceil(f.countdown)):'VALENDO!');if(f.countdown===0)toast('Valendo!','good',800);return;}f.time=Math.max(0,f.time-dt);f.cooldown=Math.max(0,f.cooldown-dt);for(const a of f.home.concat(f.away))a.cooldown=Math.max(0,a.cooldown-dt);f.ball.position.x+=f.vx*dt;f.ball.position.z+=f.vz*dt;const fr=Math.pow(.17,dt);f.vx*=fr;f.vz*=fr;f.ball.rotation.z-=f.vx*dt;f.ball.rotation.x+=f.vz*dt;const ball=f.ball.position,pd=Math.hypot(ball.x-player.x,ball.z-player.z),playerSpeed=Math.hypot(player.vx||0,player.vz||0);
    if(pd<1.35&&Math.hypot(f.vx,f.vz)<4.5&&f.cooldown<=0){const tx=player.x+Math.sin(player.facing)*.92,tz=player.z+Math.cos(player.facing)*.92;ball.x=lerp(ball.x,tx,Math.min(1,dt*10));ball.z=lerp(ball.z,tz,Math.min(1,dt*10));f.vx=(player.vx||0)*.72;f.vz=(player.vz||0)*.72;f.possession=0;}else if(pd<.9&&f.cooldown<=0){const an=Math.atan2(ball.x-player.x,ball.z-player.z);f.vx+=Math.sin(an)*(1.35+playerSpeed*.18);f.vz+=Math.cos(an)*(1.35+playerSpeed*.18);f.cooldown=.1;}
    const homeField=f.home[0],homeKeeper=f.home[1],awayField=f.away.filter(a=>a.role==='field'),awayKeeper=f.away.find(a=>a.role==='keeper');const mateTargetX=clamp(ball.x-5,f.cx-f.w/2+4,f.cx+3),mateTargetZ=clamp(ball.z+(player.z>ball.z?-3:3),f.cz-f.d/2+2,f.cz+f.d/2-2);v705AnimateAthlete(homeField,dt,mateTargetX,mateTargetZ,.95);if(Math.hypot(ball.x-homeField.group.position.x,ball.z-homeField.group.position.z)<1.1&&ball.x<f.cx+3)v705CpuKick(f,homeField,true,7.5);
    ...
    const halfW=f.w/2,halfD=f.d/2,goal=Math.abs(ball.z-f.cz)<3.35;if(ball.x>f.cx+halfW){if(goal){f.score[0]++;toast('GOL DO SEU TIME!','good',1700);v705FootballReset(-1);}else{ball.x=f.cx+halfW-.55;ball.z=clamp(ball.z,f.cz-halfD+1,f.cz+halfD-1);f.vx=-2;f.vz=0;f.cooldown=.55;toast('Tiro de meta / reposição.','warn',750);}}if(ball.x<f.cx-halfW){if(goal){f.score[1]++;toast('Gol do adversário.','warn',1500);v705FootballReset(1);}else{ball.x=f.cx-halfW+.55;ball.z=clamp(ball.z,f.cz-halfD+1,f.cz+halfD-1);f.vx=2;f.vz=0;f.cooldown=.55;}}if(ball.z>f.cz+halfD||ball.z<f.cz-halfD){const side=ball.z>f.cz?1:-1;ball.z=f.cz+side*(halfD-.55);ball.x=clamp(ball.x,f.cx-halfW+2,f.cx+halfW-2);f.vx=0;f.vz=-side*1.2;f.cooldown=.55;if(performance.now()-f.lastOutAt>900){f.lastOutAt=performance.now();toast('Lateral — bola recolocada em jogo.','warn',750);}}
    player.x=clamp(player.x,f.cx-halfW-1.2,f.cx+halfW+1.2);player.z=clamp(player.z,f.cz-halfD-1.2,f.cz+halfD+1.2);v705Hud('⚽ Futebol OTTHI',`${Math.ceil(f.time)} s • ${f.score[0]} × ${f.score[1]}${f.possession===0?' • bola com você':''}`);v705UpdateScoreboard(f.score[0],f.score[1]);v705PublishSport({type:'football',mode:f.mode,score:f.score,time:Math.ceil(f.time),ball:[+ball.x.toFixed(2),+ball.z.toFixed(2)]});if(f.time<=0){f.finished=true;const won=f.score[0]>f.score[1];addXP(won?50:22);if(won)addCoins(30);toast(won?'Vitória no futebol!':f.score[0]===f.score[1]?'Empate no futebol.':'Fim de partida.',won?'good':'warn',2200);setTimeout(()=>stopSportV704(),1700);}}
  ...
  function startCourtV704(type){stopSportV704(false);closeModal();const c=SPORTS_V705.courts.get(type);if(!c)return;c.ball=c.ball||v705Ball(type==='footvolley'?0xffd24b:0xffffff,.39);if(!c.shadow){c.shadow=new THREE.Mesh(new THREE.CircleGeometry(.48,18),renderMat(0x111827,{transparent:true,opacity:.25,roughness:1,side:THREE.DoubleSide}));c.shadow.geometry.rotateX(-Math.PI/2);worldGroup.add(c.shadow);}if(!c.team.length)c.team=[v705Athlete(0x2f7ed6,c.cx-2,c.cz-5,`${c.label} parceiro`,'field')];if(!c.opponents.length)c.opponents=[v705Athlete(0x8b5cf6,c.cx-2,c.cz+5,`${c.label} CPU 1`,'field'),v705Athlete(0xd66083,c.cx+2,c.cz+6,`${c.label} CPU 2`,'field')];c.score=[0,0];c.touches=[0,0];c.lastSide=-1;c.serving=0;c.state='serve';c.timer=150;c.vx=c.vy=c.vz=0;c.pointLock=0;c.ball.position.set(c.cx,.6,c.cz-6);player.x=c.cx+2;player.z=c.cz-6;player.y=v705GroundY(player.x,player.z);player.vx=player.vz=0;SPORTS_V705.active=c;v705SetPrimaryAction(c.foot?'🦶':'🏐',c.foot?'Chutar':'Sacar/Atacar');v705SetSpecial(c.foot?'⬆️':'🙌','Levantar');v705Hud(`${c.foot?'🏖️':'🏐'} ${c.label}`,'AÇÃO sacar/atacar • PODER levantar');toast(`${c.label} 2 × 2. Sombra no chão mostra onde a bola vai cair. Até 3 toques por lado.`, 'good',3200);}
  ...
  function courtHitV704(){const c=SPORTS_V705.active;if(!c||!['volley','footvolley'].includes(c.type))return false;const d=Math.hypot(c.ball.position.x-player.x,c.ball.position.z-player.z);if(c.state==='serve'&&c.serving===0){c.ball.position.set(player.x,1.05,player.z+.45);v705CourtSend(c,c.foot?5.0:6.6,6.5,c.cx+Math.sin(player.facing)*2);c.lastSide=0;c.touches=[1,0];return true;}if(d<2.75&&c.ball.position.y<(c.foot?2.55:3.65)&&c.ball.position.z<c.cz+.7){c.touches[0]=(c.lastSide===0?c.touches[0]+1:1);c.touches[1]=0;c.lastSide=0;if(c.touches[0]>3){v705CourtPoint(c,1);return true;}const aimX=clamp(c.ball.position.x+Math.sin(player.facing)*3,c.cx-c.w/2+1,c.cx+c.w/2-1);v705CourtSend(c,c.foot?4.8:6.25,6.9,aimX);if(!c.foot)player.vy=Math.max(player.vy||0,2.4);return true;}return false;}
  ...
  function updateCourtV704(c,dt){if(!c||SPORTS_V705.active!==c)return;c.timer=Math.max(0,c.timer-dt);c.pointLock=Math.max(0,c.pointLock-dt);if(c.state==='serve'){v705Hud(`${c.foot?'🏖️':'🏐'} ${c.label}`,`${c.serving===0?'AÇÃO para sacar':'Saque do computador'} • ${c.score[0]} × ${c.score[1]}`);return;}c.vy-=9.2*dt;c.ball.position.x+=c.vx*dt;c.ball.position.y+=c.vy*dt;c.ball.position.z+=c.vz*dt;if(c.shadow){c.shadow.visible=true;c.shadow.position.set(c.ball.position.x,.24,c.ball.position.z);const scale=clamp(1.35-c.ball.position.y*.08,.55,1.15);c.shadow.scale.setScalar(scale);c.shadow.material.opacity=clamp(.38-c.ball.position.y*.045,.1,.32);}c.ball.rotation.x+=c.vz*dt;c.ball.rotation.z-=c.vx*dt;if(Math.abs(c.ball.position.z-c.cz)<.26&&c.ball.position.y<c.netH+.13){c.ball.position.z=c.cz+(c.vz>0?-.3:.3);c.vz*=-.42;c.vy*=.58;}
    ...
    const halfW=c.w/2,halfD=c.d/2;player.x=clamp(player.x,c.cx-halfW-.8,c.cx+halfW+.8);player.z=clamp(player.z,c.cz-halfD-.8,c.cz+.3);if(ball.y<=.25){const inside=Math.abs(ball.x-c.cx)<=halfW&&Math.abs(ball.z-c.cz)<=halfD;const landedSide=ball.z>c.cz?1:0;v705CourtPoint(c,inside?(landedSide===1?0:1):(landedSide===1?1:0));return;}if(Math.abs(ball.x-c.cx)>halfW+1||Math.abs(ball.z-c.cz)>halfD+1){const side=ball.z>c.cz?1:0;v705CourtPoint(c,side===1?1:0);return;}v705Hud(`${c.foot?'🏖️':'🏐'} ${c.label}`,`${c.score[0]} × ${c.score[1]} • toques ${c.touches[c.lastSide<0?0:c.lastSide]}/3`);v705PublishSport({type:c.type,score:c.score,touches:c.touches,ball:[+ball.x.toFixed(2),+ball.y.toFixed(2),+ball.z.toFixed(2)]});if(c.timer<=0)stopSportV704();}
